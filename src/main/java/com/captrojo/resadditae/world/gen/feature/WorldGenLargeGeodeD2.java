...
public class WorldGenLargeGeodeD2 extends WorldGenLargeGeodeBase
{
	public WorldGenLargeGeodeD2(BlockMeta shiny, BlockMeta shell)
	{
		super(shiny, shell);
	}
			...
			for (int x1 = -r; x1 <= rc; x1++) {
				...
				for (int y1 = -r; y1 <= rc; y1++) {
					...
					for (int z1 = -r; z1 <= rc; z1++) {
						...
						if (!BlockHlpr.isAnyBlockNearby(world, x, y, z, rc2, ModBlocks.geode_shell_a, ModBlocks.geode_shell_b)) {
							continue;
						}
