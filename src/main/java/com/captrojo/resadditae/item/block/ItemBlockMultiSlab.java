...
public class ItemBlockMultiSlab extends ItemSlab
{
	...
	protected final BlockMultiSlab single_slab;
	protected final ISingleSlab isingleslab;
	
	protected final Block double_slab;
	protected final IDoubleSlab idoubleslab;
	...
	public ItemBlockMultiSlab(Block block)
	{
		...
		this.single_slab = (BlockMultiSlab) block;
		this.isingleslab = (ISingleSlab) block;
		
		this.double_slab = this.isingleslab.getDoubleSlab();
		this.idoubleslab = (IDoubleSlab) this.double_slab;
	}
	...
	public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float hx, float hy, float hz)
	{
		if (stack.stackSize == 0) {
			...
			int meta0 = i1 & 0x7;
			int meta1 = this.single_slab.fixMeta(meta0);
			boolean upper = (i1 & 0x8) != 0;

			if ((side == 1 && !upper || side == 0 && upper) && block == this.single_slab && meta1 == stack.getItemDamage()) {
				if (world.checkNoEntityCollision(this.double_slab.getCollisionBoundingBoxFromPool(world, x, y, z)) && world.setBlock(x, y, z, this.double_slab, meta0 | 0x8, 3)) {
	...
	protected boolean func_150946_a(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side)
	{
		...
		Block block = world.getBlock(x, y, z);
		int meta0 = world.getBlockMetadata(x, y, z) & 0x7;
		int meta1 = this.single_slab.fixMeta(meta0);

		if (block == this.single_slab && meta1 == stack.getItemDamage()) {
			if (world.checkNoEntityCollision(this.double_slab.getCollisionBoundingBoxFromPool(world, x, y, z)) && world.setBlock(x, y, z, this.double_slab, meta0 | 0x8, 3)) {
				world.playSoundEffect((double) ((float) x + 0.5F), (double) ((float) y + 0.5F), (double) ((float) z + 0.5F), this.double_slab.stepSound.func_150496_b(), (this.double_slab.stepSound.getVolume() + 1.0F) / 2.0F, this.double_slab.stepSound.getPitch() * 0.8F);
				stack.stackSize--;
			}
	...
	{
		int meta = stack.getItemDamage() & 0x7;
