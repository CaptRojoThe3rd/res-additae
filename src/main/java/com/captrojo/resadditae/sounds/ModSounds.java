package com.captrojo.resadditae.sounds;

import com.captrojo.resadditae.main.ResAdditae;

public class ModSounds
{
	public static final String TELEPORT = "mob.endermen.portal";
	
	public static final String LOG_STRIPPED = ResAdditae.ident("item.axe.strip");
	
	public static final String VAULT_OPEN = ResAdditae.ident("block.vault.activate");
	public static final String VAULT_DISPENSE = ResAdditae.ident("block.vault.eject");
	public static final String VAULT_CLOSE = ResAdditae.ident("block.vault.deactivate");
	
	public static final String SNOW_EYE_HIT = ResAdditae.ident("mob.snow_eye.hit");
	public static final String SNOW_EYE_ATTACK = ResAdditae.ident("mob.snow_eye.attack");
	
	public static final String THROWN_HALBERD_IMPACT = ResAdditae.ident("random.thrown_halberd.impact");
	
	public static final String SPELL_BULLET_TIME_ACTIVATE = ResAdditae.ident("spell.bullet_time.activate");
}
